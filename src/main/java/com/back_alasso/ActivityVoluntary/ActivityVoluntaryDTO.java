...
public record ActivityVoluntaryDTO(Long current, Long max) {
    ...
    public static ActivityVoluntaryDTO convertToDTO(Activity activity) {
        return new ActivityVoluntaryDTO(
                activity.getActivityVoluntaries() != null
                        ? activity.getActivityVoluntaries().stream()
                        .filter(ActivityVoluntary::isIs_registered) // ✅ Filtrer ceux qui sont enregistrés
                        .count()
                        : 0L,
                activity.getVolontaries_request() != null ? activity.getVolontaries_request() : 0L // ✅ Gestion du null
        );
    }
}
